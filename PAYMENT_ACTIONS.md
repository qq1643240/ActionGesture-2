...
选择顺序仍然是：先选单击/双击/长按，再选方向，最后选快捷动作。快捷动作里的“无”表示不执行微信/支付宝等自定义动作，系统自带动作仍按下方 iOS 设置执行。

动作保存在 `com.huami.actiongesture` 的 `customAction.<gesture>.<direction>` 偏好中。选择“无”表示不执行自定义快捷动作。
